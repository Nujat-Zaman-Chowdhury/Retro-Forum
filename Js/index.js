    ...
    div.innerHTML = `
        <div class="space-y-4">
                    ...
                    <div class="bg-[#797DFC1A] p-10 rounded-xl flex flex-col lg:flex-row gap-6">
                      <div class="w-[50px] lg:w-[72px] h-[50px] lg:h-[72px] rounded-2xl shadow-sm  relative bg-[url('${
                        post.image
                      ...
                      <div class="flex-1">
                          ...
                          <div class="flex gap-4 lg:gap-0 flex-col lg:flex-row justify-start lg:justify-between items-start lg:items-center">
                            <div class="text-[#12132D99] font-inter flex flex-col lg:flex-row items-start lg:items-center gap-5">
                              <div class="flex gap-3 items-center"><i class="ri-chat-2-line text-xl"></i> <span>${
                                post.comment_count
                              }?</span></div>
                              <div class="flex gap-3 items-center"><i class="ri-eye-line text-xl"></i> <span>${
                                post.view_count
                              }</span></div>
                              <div class="flex gap-3 items-center"><i class="ri-time-line text-xl"></i> <span>${
                                post.posted_time
                              } min</span></div>
                            </div>
                            <div class="">
                              <button id="mark-as-btn" onclick="add('${post.title.replace(
                                /'/g,
                                ""
                              )}', '${
      post.view_count
    }')" class="w-[28px] h-[28px] rounded-full bg-[#10B981] p-4 flex justify-center items-center mx-auto"><i class="ri-mail-open-line text-white font-bold"></i></button>
                            </div>
                          </div>
                      </div>
                    </div>
...
const add = (title, postView) => {
  ...
  const div = document.createElement("div");
  div.className =
    "bg-white shadow-sm rounded-2xl lg:flex justify-between items-center w-full p-4";
  div.innerHTML = `
    <div class="w-[212px]">
